...
class VanillaRNN(object):
    ...
    def step_forward(self, x, prev_h):
        ...
        ############################################################################
        wh=prev_h.dot(self.params[self.wh_name])
        wx=x.dot(self.params[self.wx_name])
        next_h = np.tanh(wh+wx+self.params[self.b_name])
    ...
    def step_backward(self, dnext_h, meta):
        ...
        derivative_activation= 1 - np.square(next_h)
        derivative=dnext_h*derivative_activation
        dprev_h=derivative.dot(self.params[self.wh_name].T)
        db=np.sum(derivative, axis=0)
        dx= derivative.dot(self.params[self.wx_name].T)
    ...
    def forward(self, x, h0):
        ...
        for i in range(x.shape[1]):
            prev_h, meta_t = self.step_forward(x[:,i,:], prev_h)
            h[:,i,:]=prev_h
    ...
    def backward(self, dh):
        ...
        for i in reversed(range(dh.shape[1])):
            next_h = dprev_h + dh[:,i,:]
            dx_t, dprev_h, dWx, dWh, db = self.step_backward(next_h,self.meta[i])
...
class LSTM(object):
    ...
    def step_forward(self, x, prev_h, prev_c):
        ...
        a_i=sigmoid(activation[:,0:self.h_dim])
        a_f=sigmoid(activation[:,self.h_dim:2*self.h_dim])
        a_o=sigmoid(activation[:,2*self.h_dim:3*self.h_dim])
        a_g=np.tanh(activation[:,3*self.h_dim:4*self.h_dim])
        next_c = np.multiply(a_f,prev_c) + np.multiply(a_i,a_g)
        next_h=np.multiply(a_o,np.tanh(next_c))
        meta = [x,a_i,a_f,a_o,a_g,prev_c,next_c,prev_h,next_h]
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        #############################################################################
        x,a_i,a_f,a_o,a_g,prev_c,next_c,prev_h,next_h=meta
        
        dnext_c +=  dnext_h * a_o * (1- np.square(np.tanh(next_c)))
        
        
        a_f_delta = a_f*(1-a_f)*dnext_c*prev_c
        a_i_delta = a_i * (1-a_i) *dnext_c * a_g
        a_g_delta = (1-a_g**2) *dnext_c * a_i
        a_o_delta =a_o * (1-a_o) * dnext_h * np.tanh(next_c)
        a_delta = np.concatenate((a_i_delta,a_f_delta,a_o_delta,a_g_delta),axis=1)
       
        dprev_c =  a_f * dnext_c

        dprev_h = a_delta.dot(self.params[self.wh_name].T)
        
        dx = a_delta.dot(self.params[self.wx_name].T)

        dWx = x.T.dot(a_delta)
        dWh = prev_h.T.dot(a_delta)
        db = np.sum(a_delta,axis=0)
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
    ...
    def forward(self, x, h0):
        ...
        prev_h=h0
        for i in range(x.shape[1]):
            prev_h, prev_c,meta_t = self.step_forward(x[:,i,:], prev_h,prev_c)
            h[:,i,:]=prev_h
            self.meta.append(meta_t)
        #############################################################################
        #                               END OF YOUR CODE                            #
    ...
    def backward(self, dh):
        ...
        for i in reversed(range(dh.shape[1])):
            next_h = dprev_h + dh[:,i,:]
            dx_t, dprev_h, dprev_c, dWx, dWh, db = self.step_backward(next_h,dprev_c,self.meta[i])
            self.grads[self.wx_name]+=dWx
            self.grads[self.wh_name]+=dWh
            self.grads[self.b_name]+=db
            dx[:,i,:] = dx_t
